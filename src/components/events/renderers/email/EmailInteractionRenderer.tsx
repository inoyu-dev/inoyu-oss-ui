...
export function EmailInteractionRenderer({ event, compact }: { event: UnomiEvent, compact: boolean }) {
    ...
    return (
      <div className="flex items-center space-x-2 p-4 bg-muted">
        ...
        <div className="flex-1">
          <h4 className="text-lg font-semibold">{t('Email Interaction')}</h4>
          <p className="text-muted-foreground">Campaign ID: {String(event.properties?.campaignId ?? 'N/A')}</p>
  ...
  return (
    <div className="space-y-4 p-4 border rounded-lg shadow-md bg-card">
      <h2 className="text-xl font-bold">{t('Event Type')}: {t('Email Interaction')}</h2>
      <div>
        <span className="font-semibold">{t('Campaign ID')}:</span>
        <span className="ml-2">{String(event.properties?.campaignId ?? '')}</span>
      </div>
      <div>
      ...
      <div>
        <span className="font-semibold">{t('Email ID')}:</span>
        <span className="ml-2">{String(event.properties?.emailId ?? '')}</span>
      </div>
      <div>
