...
interface ConditionType {
  ...
  // Support for flat RESTConditionType structure from API
  id?: string;
  name?: string;
  description?: string;
  tags?: string[];
...
const ConditionTypeList: React.FC = () => {
  ...
  const canDeploy = featureFlags.conditionTypeDeployment;

  // Normalize condition type from API (handles both flat and nested structures)
  const normalizeConditionType = useCallback((type: Partial<ConditionType> & Record<string, unknown>): ConditionType => {
    ...
    // Handle flat RESTConditionType structure
    return {
      ...
      targetObjectTypes: type.targetObjectTypes,
    };
  }, []);

  const fetchConditionTypes = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      const normalized = rawData.map(normalizeConditionType);
      setConditionTypes(normalized);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch condition types');
  ...
  const filteredTypes = conditionTypes.filter(type => {
    ...
    const matchesScope = !filterScope || type.scope === filterScope;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-2">Loading condition types...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">{t('Condition Types')}</h1>
          {!canDeploy && (
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{t('Condition Types')}</CardTitle>
            <div className="flex items-center space-x-2">
              <div className="relative">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-text" />
                <Input
                  placeholder={t('Search condition types...')}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-8 w-64"
                />
              </div>
              <Button variant="outline" size="sm" onClick={fetchConditionTypes}>
        ...
        <CardContent>
          {filteredTypes.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {searchTerm ? t('No condition types match your search') : t('No condition types found')}
            </div>
          ) : (
                      ...
                      <div className="flex items-center space-x-3 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 mt-2 text-sm text-muted-foreground">
                            ...
                            {type.targetObjectTypes && type.targetObjectTypes.length > 0 && (
                              <span>Targets: {type.targetObjectTypes.join(', ')}</span>
                            )}
                          </div>
                        </div>
                      </div>
        ...
        <Dialog open={!!selectedType} onOpenChange={() => setSelectedType(null)}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              {selectedType.targetObjectTypes && selectedType.targetObjectTypes.length > 0 && (
                <div>
                  <h3 className="font-medium mb-2">{t('Target Object Types')}</h3>
                  <div className="flex flex-wrap gap-2">
                    {selectedType.targetObjectTypes.map((target) => (
